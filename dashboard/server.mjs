...
const SESSION_STALLED_THRESHOLD_MS = 15 * 60 * 1000;

function classifyPulse(session, isSpawning = false, isKilling = false) {
  if (isSpawning) return 'starting';
  if (isKilling) return 'killing';
  const hasBackend = session.status === 'active' && session.backendActive;
  if (!hasBackend) return 'unborn';

  const lastMs = session.lastInteractionMs;
  if (lastMs === null || lastMs < SESSION_ACTIVE_THRESHOLD_MS) return 'active';
  if (lastMs < SESSION_STALLED_THRESHOLD_MS) return 'idle';
  return 'stalled';
}

function sshContextSummary() {
  const fields = ['SSH_CONNECTION', 'SSH_CLIENT', 'SSH_TTY']
...
function slotRuntimePaths(slotName) {
  ...
  return {
    ...
    zshenvFile: path.join(currentDir, '.zsh_atc', '.zshenv'),
    zprofileFile: path.join(currentDir, '.zsh_atc', '.zprofile'),
    zshrcFile: path.join(currentDir, '.zsh_atc', '.zshrc'),
    zloginFile: path.join(currentDir, '.zsh_atc', '.zlogin'),
  };
}
...
async function tmuxSessionExists(sessionName) {
  if (!ENABLE_TMUX_BACKEND) return true;
  const result = await runCommand(TMUX_BIN, ['has-session', '-t', sessionName], 3000);
  return !!result.ok;
}
...
function buildAtcZshrc(env) {
  return [
    ...
    'fi',
    '',
    '# Preserve user-local CLI installs even when login startup files reset PATH.',
...
function buildAtcStartupShim(targetEnvVar) {
  return [
    '#!/usr/bin/env zsh',
    '# Generated by AI Traffic Control to preserve user zsh startup files.',
    'if [[ "${ATC_SOURCE_USER_ZSHRC:-1}" != "0" && -n "${' + targetEnvVar + ':-}" && -r "${' + targetEnvVar + '}" ]]; then',
    '  source "${' + targetEnvVar + '}"',
    'fi',
    '',
  ].join('\n');
...
async function ensureSlotRuntime(slotName, runId, workdir, sessionState = {}) {
  ...
  const hookEnv = {
    ...
    ATC_USER_ZSHENV: USER_ZSHENV_PATH,
    ATC_USER_ZPROFILE: USER_ZPROFILE_PATH,
    ATC_USER_ZSHRC: USER_ZSHRC_PATH,
    ATC_USER_ZLOGIN: USER_ZLOGIN_PATH,
  ...
  await fs.writeFile(paths.zshenvFile, buildAtcStartupShim('ATC_USER_ZSHENV'), { mode: 0o644 });
  await fs.writeFile(paths.zprofileFile, buildAtcStartupShim('ATC_USER_ZPROFILE'), { mode: 0o644 });
  await fs.writeFile(paths.zshrcFile, buildAtcZshrc(hookEnv), { mode: 0o644 });
  await fs.writeFile(paths.zloginFile, buildAtcStartupShim('ATC_USER_ZLOGIN'), { mode: 0o644 });
...
async function getMergedSessions() {
  ...
  const merged = await Promise.all(
    cfg.map(async (slot) => {
      ...
      const tmuxSessionName = tmuxSessionNameForSlot(slot.name);
      const [derived, tmuxPaneState, tmuxAlive] = await Promise.all([
        readJsonSafe(runtime.derivedFile, null),
        readTmuxSlotPaneState(slot.name),
...
<head>
  ...
  <script>
    ...
    const SESSION_ACTIVE_THRESHOLD_MS = ${SESSION_ACTIVE_THRESHOLD_MS};
    const SESSION_STALLED_THRESHOLD_MS = ${SESSION_STALLED_THRESHOLD_MS};
    ...
    function sessionCard(s) {
      ...
      const lastMs = s.lastInteractionMs;

      let sessionState = 'unborn';
      if (isSpawning) {
        sessionState = 'starting';
      } else if (isKilling) {
        sessionState = 'killing';
      } else if (hasBackend) {
        if (lastMs === null || lastMs < SESSION_ACTIVE_THRESHOLD_MS) {
          sessionState = 'active';
        } else if (lastMs < SESSION_STALLED_THRESHOLD_MS) {
          sessionState = 'idle';
        } else {
      ...
      const isUnborn = sessionState === 'unborn';
      const isStalled = sessionState === 'stalled';
      ...
      const actionClass = isKilling ? 'color-killing' : (isSpawning ? 'color-starting' : (isActive ? 'color-active' : (isIdle ? 'color-idle' : (isStalled ? 'color-stalled' : 'color-unborn'))));
